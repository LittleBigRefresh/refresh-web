  ...
  template: `
    <div class="group relative">
      ...
      <div class="absolute z-[1] w-64 px-5 py-2.5 hidden group-hover:flex flex-col rounded-b bg-header-background gap-y-2"
        ...
        <app-navbar-item *ngFor="let link of category.items" [href]="link.route" [icon]="link.icon!" [title]="link.name" [iconClass]="'w-4 text-[1.1rem]'" labelClass="text-lg"></app-navbar-item>
      </div>
    </div>
  `
})
